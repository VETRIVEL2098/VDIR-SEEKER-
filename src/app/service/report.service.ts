...
async generatePdf(reportName:any,data:any,groupBy:any,titles?:any, extra?:any) {
...
async generateCSV(reportName:any,data:any) {
    this.dataService.loadReportConfigJson(reportName).subscribe(config=>{
      if (config == null) {
        this.dialogService.openSnackBar('Sorry! Something went wrong', 'OK');
        return
...
private loadContent(data:any,groupBy?:any,titles?:any) {
  var content:any =[]
  ...
  if (grpDef!=null) {
      ...
      var groupField:any = (item:any) => item[grpDef.colName]
      if (grpDef.type == "date") {
          //group by specific field with required format
          groupField = (item:any) => moment(item[grpDef.colName]).format(grpDef.format);
      }
  }
...
private getPageHeader(data:any,key:any,grpCol:any,layoutColumns:any,titles?:any) {
  ...
  if (grpCol!=null && grpCol.headFormat) {
      if (grpCol.type == "date") {
        ...
        if (typeof(grpCol.headFormat)=== 'string') {
          ...
          grpCol.headFormat.forEach((e:any) => {
            subTitles.push(
...
private getAdditionalData(data:any,layoutColumns:any) {
 //header text top margin adjustment - 4 points per line
...
private getPageData(data:any,key:any,grpCol: any,titles: any) {
  var tabledata = []
  var idx=1
  ...
  for(var c=1;c<layoutColumns;c++) {
    colWidths.forEach((e:any) => widths.push(e))
  }
  ...
  if(this.additionalData) {
    var additionalData:any = this.getAdditionalData(this.additionalData,layoutColumns)
  ...
  for(var r=1;r<=data[key].length;r++) {
    var doc:any = data[key][r-1]
    var v:any = this.getRowValue(dataCols,doc,r)
...
private getTableHeader(headCols:any,layoutColumns:any) {
   var data:any = []
  for(var idx=0;idx<layoutColumns;idx++) {
    headCols.forEach((element:any) => {
      let cell:any = {
        text: element,
        style: 'tableHeader',
        bold: true,
...
// private async getImage(url:any) {
